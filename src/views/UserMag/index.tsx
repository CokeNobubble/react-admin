...
const UserMag: FC = (): ReactElement => {
  ...
  const columns: ColumnsType<ITable> = [
    ...
    },
    {
      ...
      key: "sex",
      render: (_, record) => (
        <Tag color={record.sex === "男" ? "blue" : "red"}>{record.sex}</Tag>
      ),
    },
    {
      title: "手机号",
  ...
  const handleEditRow = (record: ITable): void => {
    // todo
    setId(record.id);
    ...
    <>
      <div className="flex justify-between items-center mb-30px shadow p-20px">
        <div className="flex gap-20px items-center">
          ...
          <Input
            className="w300px"
            onKeyDown={handleSearchByDown}
            ref={inputRef}
            placeholder="用户名/手机号"
          />
          <Button
            type="primary"
            onClick={() => handleSearch(inputRef.current!.input!.value)}
          >
            搜索
          </Button>
